...
class block(nn.Module):
    def __init__(self, in_ch_size, out_ch_size, block_type=0):
        super(block, self).__init__()
        self.bn1 = nn.BatchNorm2d(in_ch_size)
        self.bn2 = nn.BatchNorm2d(out_ch_size)

        self.relu = nn.ReLU()
        ### Type이 0이면 stride 단위 1로 ###
        if block_type == 0:
            self.conv1 = nn.Conv2d(in_ch_size, out_ch_size, kernel_size=3, stride=1, padding=1)
        ### Type이 1이면 stride 단위 2로 ###
        elif block_type == 1:
            self.conv1 = nn.Conv2d(in_ch_size, out_ch_size, kernel_size=3, stride=2, padding=1)
            self.conv_shortcut = nn.Conv2d(in_ch_size, out_ch_size, kernel_size=1, stride=2, padding=0)
        self.conv2 = nn.Conv2d(out_ch_size, out_ch_size, kernel_size=3, stride=1, padding=1)
